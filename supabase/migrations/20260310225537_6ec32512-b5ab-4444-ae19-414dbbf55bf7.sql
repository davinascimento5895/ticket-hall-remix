...
-- C02: Server-side price validation RPC
CREATE OR REPLACE FUNCTION create_order_validated(
  p_tier_ids uuid[],
  p_quantities int[],
  p_buyer_id uuid,
  p_coupon_code text DEFAULT NULL,
  p_promoter_event_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_coupon_code IS NOT NULL AND p_coupon_code != '' THEN
    ...
    IF v_coupon_id IS NOT NULL THEN
      -- Calculate applicable subtotal (only tiers in applicable_tier_ids, or all if null)
