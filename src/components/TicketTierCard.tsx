...
export function TicketTierCard({
  id, name, description, price, originalPrice, quantityTotal, quantitySold,
  quantityReserved = 0, minPerOrder, maxPerOrder, tierType,
  salesStartDate, salesEndDate, eventId, onAdd, className,
}: TicketTierCardProps) {
  const [quantity, setQuantity] = useState(minPerOrder);
  const [joiningWaitlist, setJoiningWaitlist] = useState(false);
  // A10: Double-click protection
  ...
  // A10: Debounced add with protection
  ...
  const handleJoinWaitlist = async () => {
    ...
    try {
      await joinWaitlist({ event_id: eventId, tier_id: id, user_id: user.id, email: user.email || "" });
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 space-y-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {isLow && !isSoldOut && !isNotYetOnSale && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-warning"><AlertCircle className="h-3 w-3" /> Últimas unidades</span>
          )}
          {isSoldOut && <span className="text-xs font-medium text-destructive">Esgotado</span>}
          {isNotYetOnSale && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-muted-foreground"><Clock className="h-3 w-3" /> Vendas em {formatCountdown(salesStart!)}</span>
          )}
          {isSalesEnded && !isSoldOut && <span className="text-xs font-medium text-muted-foreground">Vendas encerradas</span>}
        </div>
      ...
      {!isDisabled && (
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 border border-border rounded-md">
            <button onClick={decrement} disabled={quantity <= minPerOrder} className="p-2 text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors">
              <Minus className="h-4 w-4" />
            </button>
            <span className="w-8 text-center text-sm font-medium tabular-nums">{quantity}</span>
            <button onClick={increment} disabled={quantity >= Math.min(maxPerOrder, available)} className="p-2 text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors">
      ...
      {isSoldOut && eventId && (
        <Button variant="outline" size="sm" className="gap-1.5" onClick={handleJoinWaitlist} disabled={joiningWaitlist}>
